...
- Mermaid is now bundled with the plugin instead of being loaded from the jsdelivr CDN, so diagrams work without an external request (better privacy, reliability, and offline support).
- Tested up to WordPress 7.0.

### Fixed

- The PHPUnit suite and PHPStan now actually execute in CI. The `ABSPATH` direct-access guard in `src/functions.php` was terminating both tools during the composer autoload bootstrap (the file was registered under `autoload.files`), so they silently exited `0`. The file is now required from the plugin bootstrap instead of being composer-autoloaded, so its guard no longer runs under CLI. PHPUnit and the official WordPress Plugin Check were added to CI so this cannot regress unnoticed.
...
- Require djot-php ^0.1.24 with improved round-trip support
